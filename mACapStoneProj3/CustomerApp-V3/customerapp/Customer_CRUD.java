...
public class Customer_CRUD {
    ...
    public static int listCustomers() throws Exception {
        ...
        printFooter();
        CustomerDAO.listCustomer().stream().forEach(System.out::println); // default toString 
     //   CustomerDAO.listCustomer().stream().forEach(cnsmr -> cnsmr.toStringv1());  // v1 -> doing S.o.println 
     //   CustomerDAO.listCustomer().stream().forEach(cnsmr -> System.out.println(cnsmr.toStringv2())); // return String 
        printFooter();
        return 1;
    }    
