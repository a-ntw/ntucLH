...
LOOP 
    if emp_record.salary < 5000 
        AND ( emp_record.manager_id = 101 
            OR emp_record.manager_id = 124 )
    then 
...
BEGIN
    ...
    loop
        ...
        OPEN c_dept_cursor(v_current_deptno);
            LOOP
                FETCH c_emp_cursor INTO
                    v_ename,v_job,v_hiredate, v_sal;
                EXIT WHEN c_emp_cursor%notfound;
                dbms_output.put_line(v_ename || 'some more');
            end loop;
            CLOSE c_emp_cursor;
    END LOOP;
    CLOSE c_dept_cursor;
...
-- 09-1.  //// error on numbering

-- from 6.1
SET SERVEROUTPUT ON

DECLARE
    v_enam  employees.last_name%TYPE;
...
practice 10
``` sql
/*Practices 10 - Stored Procedures and Funtion*/
...
CREATE PROCEDURE greet IS -- was DECLARE
    v_today      DATE := SYSDATE;
    v_tomorrow   v_today%TYPE;
BEGIN
    v_tomorrow := v_today + 1;
